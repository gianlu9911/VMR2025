...
# ---- NUOVO: DIZIONARIO ABBREVIAZIONI ----
SHORT_NAMES = {
    'stylegan1': 'SG1',
    'stylegan2': 'SG2',
    'sdv1_4': 'SD1.4',
...
for step in STEPS_ORDER:
    ...
    # Dimensioni che puoi aggiustare se assembli la griglia esternamente
    plt.figure(figsize=(10, 8)) 
    plotted_real = False 
    plotted_something = False

    for csv_file in csv_files:
        # Estrae il nome pulito dal file
        ...
        if n_fake_to_plot > 0:
            ...
            short_name = SHORT_NAMES.get(test_set_name, test_set_name) # Usa il nome abbreviato
            ...
            plt.scatter(
                ...
                label=f'{short_name} (Acc: {acc_fake:.1%})', zorder=2
            )
            plotted_something = True

    if plotted_something:
        # Linea decisionale a 0.5
        plt.plot([-0.1, 1.1], [1.1, -0.1], color='gray', linestyle='--', alpha=0.5, label='Decision Bound.')

        # TITOLO RIMOSSO: andrà nella caption della figura finale.
        ...
        # LEGENDA IN BASSO
        # Disposta orizzontalmente sotto il grafico su più colonne (ncol)
        plt.legend(
            loc='upper center', 
            bbox_to_anchor=(0.5, -0.12), 
            ncol=4, 
            fontsize='small',
            frameon=False # Rimuovere la cornice aiuta a pulire l'estetica se sono tanti
        )
        
        plt.tight_layout()

        # Usare bbox_inches='tight' anche qui assicura che la legenda non venga "tagliata"
        plt.savefig(os.path.join(OUTPUT_DIR, f"step_{step}_probs.png"), dpi=300, bbox_inches='tight')
...
print(f"\n[FINE] Boom! Layout ottimizzato per la visualizzazione a 6 pannelli.")