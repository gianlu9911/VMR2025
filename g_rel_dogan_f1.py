...
def compute_bwt_fwt(csv_path, order_list):
    """Calcola in automatico BWT e FWT leggendo il CSV appena generato."""
    ...
    # Costruiamo la matrice R (T x T) delle accuratezze
    for i, t_train in enumerate(order_list):
        for j, t_eval in enumerate(order_list):
            val = df[(df['Train_Task'] == t_train) & (df['Eval_Task'] == t_eval)]['Accuracy'].values
            if len(val) > 0:
                R[i, j] = val[0]
                
    acc = np.mean(R[-1, :]) if T > 0 else 0
    bwt = np.mean([R[-1, i] - R[i, i] for i in range(T - 1)]) if T > 1 else 0
    fwt = np.mean([R[j-1, j] for j in range(1, T)]) if T > 1 else 0
    
    print("\n" + "="*50)
    print("=== CONTINUAL LEARNING FINAL METRICS ===")
    print(f"Final Average Accuracy (ACC): {acc:.4f}")
    print(f"Backward Transfer (BWT):      {bwt:.4f}")
    print(f"Forward Transfer (FWT):       {fwt:.4f}")
    print("="*50 + "\n")
    
    # Salva un piccolo file di riepilogo
    with open(csv_path.replace(".csv", "_bwt_fwt_summary.txt"), "w") as f:
        f.write(f"Order: {order_list}\n")
...
def fine_tune_cl(
    ...
    **kwargs
    ...
    res_dir = os.path.join("results_dogan_faces", f"train_on_{fine_tuning_on}")
    os.makedirs(res_dir, exist_ok=True)

    header_needed = not os.path.exists(csv_log_path)
    
    # Apriamo in modalitá append per salvare tutte le metriche (Long Format)
    with open(csv_log_path, 'a') as f:
        if header_needed:
            f.write("Train_Task,Eval_Task,Accuracy,Precision,Recall,F1_Score,AUC\n")
        ...
        for t_task in order_list:
            ...
            with torch.no_grad():
                for batch_f, batch_l in t_loader:
                    batch_f = batch_f.to(device_obj)
                    with torch.cuda.amp.autocast():
                        outputs = classifier(batch_f)
            ...
            print(f"Task {t_task:20} | Acc: {acc:.4f} | F1: {f1:.4f} | AUC: {auc:.4f} {mark}")
            
            # Salva riga CSV
            row = f"{fine_tuning_on},{t_task},{acc:.5f},{prec:.5f},{rec:.5f},{f1:.5f},{auc:.5f}\n"
            f.write(row)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--csv_log', type=str, default='results_dogan_faces/cl_detailed_metrics.csv')
    ...
    # 4. Calcolo Automatico BWT e FWT a fine esecuzione!
    compute_bwt_fwt(args.csv_log, order_list)