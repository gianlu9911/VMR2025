...
from src.utils import BalancedBatchSampler, RelativeRepresentation, extract_and_save_features, evaluate
from src.g_utils import save_features_only

# ---------------------------------------------
# Definizione del nuovo Classificatore MLP
# ---------------------------------------------
class RelClassifierMLP(nn.Module):
    def __init__(self, rel_module, num_anchors, num_classes=2, hidden_dim=512):
        super(RelClassifierMLP, self).__init__()
        self.rel_module = rel_module
        
        # Multi-Layer Perceptron al posto del singolo strato lineare
    ...
    def forward(self, x):
        # Calcola la rappresentazione relativa rispetto alle ancore
        x_rel = self.rel_module(x)
        # Passa la rappresentazione relativa attraverso l'MLP
        return self.mlp(x_rel)
...
def fine_tune(
    ...
    run_id: str = "Run_0"  # <-- PARAMETRO PER IDENTIFICARE L'ORDINE
):
    ...
    # Directory for ROC plots
    roc_dir = os.path.join('logs_original_dataset', 'roc_plots')
    ...
    # --- CLASSIFICATORE MLP ---
    # Sostituito RelClassifier originale con il nostro RelClassifierMLP
    classifier = RelClassifierMLP(rel_module, anchors.size(0), num_classes=2).to(device)
    ...
    # Dictionaries to store ROC data for the current training step
    roc_data = {}

    # --- EVALUATION LOOP (CON AMP E METRICHE SCIKIT-LEARN) ---
    classifier.eval()
    
    # Inizializza il plot ROC
    ...
    for name, ds in dataloaders_test.items():
        ...
        with torch.no_grad():
            for b_f, b_l in test_loader:
                ...
                with torch.cuda.amp.autocast():
                    ...
                    # Estraiamo le probabilità per l'AUC (classe 1 - fake)
        ...
        try:
            ...
            # Calcola fpr e tpr per il plot
            fpr, tpr, _ = roc_curve(all_labels, all_probs)
            roc_data[name] = {'fpr': fpr, 'tpr': tpr, 'auc': roc_auc}
            
            # Aggiungi curva al plot
            plt.plot(fpr, tpr, lw=2, label=f'{name} (AUC = {roc_auc:.3f})')
            
        except ValueError:
            roc_auc = 0.0 # Sicurezza in caso di classi mancanti
        ...
        print(f"  -> Acc: {acc:.4f} | F1: {f1:.4f} | AUC: {roc_auc:.4f} | Recall: {rec:.4f} | Prec: {prec:.4f}")

    # Finalizza e salva il plot ROC
    plt.plot([0, 1], [0, 1], color='navy', lw=2, linestyle='--')
    plt.xlim([0.0, 1.0])
    plt.ylim([0.0, 1.05])
    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive Rate')
    plt.title(f'ROC Curve after training on {fine_tuning_on} ({run_id})')
    plt.legend(loc="lower right")
    
    # Salva la figura
    roc_plot_filename = os.path.join(roc_dir, f'roc_{run_id}_{fine_tuning_on}.png')
    plt.savefig(roc_plot_filename)
    plt.close() # Chiudi la figura per liberare memoria
    print(f"Salvataggio plot ROC in: {roc_plot_filename}")

    # --- SALVATAGGIO CSV CON IDENTIFICATIVO RUN ---
    if eval_csv_path is None:
        eval_csv_path = os.path.join('logs_mlp_original_dataset', 'detailed_metrics_results.csv')
