...
# --- Avalanche imports (used only if user enables --use_avalanche) ---
try:
    from avalanche.benchmarks import dataset_benchmark
    from avalanche.benchmarks.utils import AvalancheDataset
    from avalanche.training import Naive
    from avalanche.training.plugins import EvaluationPlugin
    from avalanche.evaluation.metrics import accuracy_metrics, bwt_metrics, forward_transfer_metrics
...
def fine_tune(args, backbone_name=None, fine_tuning_on=None):
    ...
    rel_module = RelativeRepresentation(anchors.to(get_device(args.device)))
    ...
    # Classifier
    classifier = RelClassifier(rel_module, anchors.size(0), num_classes=2).to(get_device(args.device))
    criterion = nn.CrossEntropyLoss().to(get_device(args.device))
    optimizer = torch.optim.Adam(classifier.parameters(), lr=args.lr)

    # Training loop
    start_time = time.time()
    for epoch in range(args.epochs):
        train_loss, train_acc = train_one_epoch(classifier, feat_loader, criterion, optimizer, get_device(args.device))
    ...
    for name, dataset in dataloaders_test.items():
        ...
        if args.force_recompute_features or not os.path.exists(feat_file_test):
            ...
            feats_test, labels_test, feat_time = extract_and_save_features(backbone, loader,
                                                                          feat_file_test, get_device(args.device), split='test_set')
        else:
        ...
        loss, acc = evaluate(classifier, test_loader, criterion, get_device(args.device),
...
# ---------------------------------------------
# Avalanche integration: run on cached/extracted features
# ---------------------------------------------
def run_avalanche_on_features(args, tasks_order=None):
    """
    Build per-task feature TensorDatasets (train+test) using your backbone & extractor,
    create an Avalanche benchmark, train a RelClassifier with Naive strategy and EvaluationPlugin,
    and print ACC / BWT / FWT results.
    """
    if not AVALANCHE_AVAILABLE:
        raise RuntimeError("Avalanche is not available. Install avalanche-lib to use this feature (pip install avalanche-lib).")

    device = get_device(args.device)
    print(f"[Avalanche] using device: {device}")

    # default task order
    tasks_order = tasks_order or ['stylegan1', 'stylegan2', 'stylegan_xl', 'sdv1_4']
    real_dir = IMAGE_DIR['real']

    # load backbone (feature extractor) same as your pipeline
    backbone = load_pretrained_model(PRETRAINED_MODELS[args.backbone])
    backbone.resnet.fc = nn.Identity()
    backbone.to(device).eval()

    # We'll store per-task TensorDatasets
    train_datasets = []
    test_datasets = []

    # Also keep one train real-features pool to sample anchors (use first task's train real samples)
    anchors_pool = None

    feat_dir = f"./feature_{args.backbone}"
    os.makedirs(feat_dir, exist_ok=True)

    for t in tasks_order:
        fake_dir = IMAGE_DIR[t]
        # prepare dataloaders for feature extraction
        train_ds = RealSynthethicDataloader(real_dir, fake_dir)  # default = train set
        test_ds  = RealSynthethicDataloader(real_dir, fake_dir, split='test_set')
        ...
        # file paths to cache features
        train_feat_file = os.path.join(feat_dir, f"av_train_real_vs_{t}_features.pt")
        test_feat_file  = os.path.join(feat_dir, f"av_test_real_vs_{t}_features.pt")

        # extract or load train features
        if args.force_recompute_features or not os.path.exists(train_feat_file):
            feats_train, labels_train, _ = extract_and_save_features(backbone, train_loader, train_feat_file, device, split='train_set')
        else:
            dd = torch.load(train_feat_file)
            feats_train, labels_train = dd["features"], dd["labels"]
            print(f"[Avalanche] Loaded cached train features for {t}")

        # extract or load test features
        if args.force_recompute_features or not os.path.exists(test_feat_file):
            feats_test, labels_test, _ = extract_and_save_features(backbone, test_loader, test_feat_file, device, split='test_set')
        else:
            dd = torch.load(test_feat_file)
            feats_test, labels_test = dd["features"], dd["labels"]
            print(f"[Avalanche] Loaded cached test features for {t}")

        # Build TensorDatasets (features on CPU, labels long)
        train_td = TensorDataset(feats_train.cpu(), labels_train.cpu().long())
        test_td  = TensorDataset(feats_test.cpu(), labels_test.cpu().long())

        train_datasets.append(train_td)
        test_datasets.append(test_td)

        # pick anchors pool from the first task's real training features
        if anchors_pool is None:
            real_mask = (labels_train == 0)
            if real_mask.sum().item() == 0:
                raise RuntimeError(f"No real samples in train set for task {t} -> cannot create anchors.")
            anchors_pool = feats_train[real_mask].cpu()

    # Build Avalanche CL benchmark (one experience per task)
    scenario = dataset_benchmark(train_datasets=train_datasets, test_datasets=test_datasets)
    print(f"[Avalanche] Created scenario with {len(list(scenario.train_stream))} experiences")

    # Build anchors (sample from anchors_pool)
    if args.num_anchors is not None:
        rng = torch.Generator().manual_seed(args.seed)
        num_requested = int(args.num_anchors)
        if num_requested > len(anchors_pool):
            idx = torch.randint(low=0, high=len(anchors_pool), size=(num_requested,), generator=rng)
        else:
            idx = torch.randperm(len(anchors_pool), generator=rng)[:num_requested]
        anchors = anchors_pool[idx].to(device)
    else:
        anchors = anchors_pool.to(device)

    print(f"[Avalanche] Using {anchors.size(0)} anchors")

    # Build relative module + classifier (this model will be trained by Avalanche)
    rel_module = RelativeRepresentation(anchors.to(device))
    model = RelClassifier(rel_module, anchors.size(0), num_classes=2).to(device)

    criterion = nn.CrossEntropyLoss().to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)

    # Avalanche evaluation plugin with accuracy, BWT, FWT
    ...
    print("[Avalanche] Starting sequential training...")
    last_results = None
    for experience in scenario.train_stream:
        print(f"[Avalanche] Training on experience: {experience.current_experience}/{len(list(scenario.train_stream))-1}")
        cl_strategy.train(experience)
        # Evaluate on all test experiences (this updates eval_plugin)
        last_results = cl_strategy.eval(scenario.test_stream)

    # last_results is a dict with many metric keys -> print keys and extract common ones
    print("\n[Avalanche] Evaluation results keys:")
    for k in sorted(list(last_results.keys())):
        print("  ", k)

    # helper to fetch metric value by substring
    def fetch_metric(d, substr):
        for k, v in d.items():
            if substr in k:
                return v
        return None

    acc = fetch_metric(last_results, "Top1_Acc_Stream")
    bwt = fetch_metric(last_results, "Backward_Transfer") or fetch_metric(last_results, "BWT_Stream") or fetch_metric(last_results, "BWT")
    fwt = fetch_metric(last_results, "Forward_Transfer") or fetch_metric(last_results, "FWT_Stream") or fetch_metric(last_results, "FWT")

    print("\n=== Avalanche aggregated metrics (best-effort lookup) ===")
    print(f"ACC (Top1_Acc_Stream): {acc}")
    print(f"BWT: {bwt}")
    print(f"FWT: {fwt}")

    # Optionally, save the trained classifier state
    ckpt = os.path.join("./checkpoint", f"av_relclf_{args.backbone}_on_{'_'.join(tasks_order or ['stylegan1','stylegan2','stylegan_xl','sdv1_4'])}.pth")
    torch.save({'state_dict': model.state_dict()}, ckpt)
    print(f"[Avalanche] Saved trained model to {ckpt}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use_avalanche', action='store_true',
                        help="If set, run the Avalanche continual-learning flow on features (train sequentially on tasks).")
    args = parser.parse_args()

    if args.use_avalanche:
        if not AVALANCHE_AVAILABLE:
            raise RuntimeError("Avalanche library not installed. Install with `pip install avalanche-lib`.")
        tasks_order = ['stylegan1', 'stylegan2', 'stylegan_xl', 'sdv1_4']
        print("Running Avalanche continual learning on tasks:", tasks_order)
        results = run_avalanche_on_features(args, tasks_order=tasks_order)
        print("Avalanche run finished.")
    else:
        results = fine_tune(args)
        print("Standard fine-tune finished.")

    print("\nAll test results (if any):")
    if isinstance(results, dict):
        for k, v in results.items():
            if isinstance(v, dict):
                print(f" - {k}: loss={v['loss']:.4f}, acc={v['acc']:.4f}, feat_time={v.get('feat_time', 0):.2f}s")
            else:
                print(f" - {k}: {v}")
    else:
        print(results)
